...
abstract class OAuth2 {
  ...
  public function grantAccessToken() {
    ...
    if(!$input)$input = filter_input_array(INPUT_GET, $filters);
    ...
    switch ($input["grant_type"]) {
      case OAUTH2_GRANT_TYPE_AUTH_CODE:
        if (!$input["code"] || !$input["redirect_uri"])
          $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'need code and redirect_uri');
      ...
      case OAUTH2_GRANT_TYPE_ASSERTION:
        if (!$input["assertion_type"] || !$input["assertion"])
          $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'assertion type is missing');
      ...
      case OAUTH2_GRANT_TYPE_NONE:
        ...
        if ($stored === FALSE)
          $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'unable to get auth code');
    }
